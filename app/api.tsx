...
export const postJoinEvent = ({ event_id: eventId }: { event_id: string }) => {
  return AuthAPI.post(`/events/${eventId}/attendances/apply`);
};

export const deleteCancelEvent = ({
  event_id: eventId,
}: {
  event_id: string;
}) => {
  return AuthAPI.delete(`/events/${eventId}/attendances/cancel`);
};

export const getEventAttendances = ({
  event_id: eventId,
}: {
  event_id: string;
}) => {
...
export const patchRole = ({ uid, role }: { uid: string; role: string }) => {
  return AuthAPI.patch("/admin/users/change_role", {
    uid: uid,
    role: role,
  });
};
...
export const deleteEvent = ({ eventId }: { eventId: string }) => {
  return AuthAPI.delete(`/events/${eventId}`);
};

export const getApplyEvent = () => {
  return AuthAPI.get(`/my_attended_events`);
};
...
export const patchUser = ({ uid, birth, name, isMarketing, phone }: any) => {
  return AuthAPI.patch(`/admin/users/${uid}`, {
    birth: birth,
    name: name,
...
export const deleteUser = ({ uid }: any) => {
  return AuthAPI.delete(`/admin/users/${uid}`);
};
